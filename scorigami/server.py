...
Run:
    python3 scorigami/server.py [--port 8765]

The server queries scorigami.sqlite directly with window functions so any
...
class Handler(SimpleHTTPRequestHandler):
    # Serve files from the scorigami/ directory regardless of cwd
    ...
    def do_GET(self):
        ...
        if url.path == "/tally":
            ...
            return
        # Default to cube.html for the bare path
        if url.path in ("/", ""):
            self.path = "/cube.html"
        return super().do_GET()
...
def main():
    ...
    print(f"scorigami server on http://{args.host}:{args.port}/")
